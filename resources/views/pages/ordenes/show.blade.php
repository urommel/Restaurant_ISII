        ...
        <div class="grid grid-cols-1 gap-8 md:grid-cols-3">
                ...
                <div class="divide-y-2">
                    ...
                    <form action="{{ route('orden.store') }}" method="POST" role="form">
                                ...
                                <tbody id="tablaProductosBody">
                                    ...
                                    @foreach ($orden->detalles as $detalle)
                                        <tr>
                                            <td>
                                                <button
                                                    onclick="eliminarProducto('{{ $detalle->producto_nombre }}')">Eliminar</button>
                                            </td>
                                            <td>{{ $detalle->producto_nombre }}</td>
                                            <td>
                                                <input type="number" id="quantity_{{ $detalle->producto_nombre }}"
                                                    name="cantidades[]" min="1" value="{{ $detalle->cantidad }}"
                                                    class="w-16 px-2 py-1 ml-4 text-sm text-gray-900
                                                bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700
                                                dark:text-gray-400"
                                                    oninput="actualizarCantidad('{{ $detalle->producto_nombre }}', this.value, {{ $detalle->precio_unitario }})">
                                            </td>
                                            <td>${{ $detalle->precio_unitario }}</td>
                                            <td id="precioTotal_{{ $detalle->producto_nombre }}">
                        ...
                        <div id="montoTotalContainer" class="mt-4 text-right" style="display: none;">
                            <span id="montoTotalLabel"
                                class="text-lg font-bold text-gray-700 dark:text-gray-400">Monto
                                Total:</span>
                            <span id="montoTotal"
            ...
            <div class="md:col-span-1 ">
                ...
                <div class="p-4 bg-white rounded-lg dark:bg-gray-800">
                    ...
                    <div class="px-4 py-3 bg-transparent border-b card-header">
                        <label for="client_id">Cliente: {{ $orden->cliente->name }}</label>
                    </div>
                    <div class="px-4 py-3 bg-transparent border-b card-header">
                    ...
                    <div class="p-4 card-body">
                        ...
                        <div id="carrito">
                            ...
                            <p class="mb-4">Total: $<span id="total">{{ $orden->monto_total }}</span></p>
                        </div>
                    ...
                    <div id="pagoFormContainer">
                        <form id="pagoForm" action="{{ route('orde.pagar', $orden->id) }}" method="POST">
                            @csrf
                            <input type="hidden" name="_method" value="PATCH">
                            <button type="submit" id="botonPagar"
                                class="w-full mt-4 text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                Pagar
                            </button>
                        </form>
                    </div>
...
<script>
    ...
    function eliminarProducto(nombreProducto) {
        // Encuentra la fila con el nombre del producto y elimínala
        const filaEliminar = document.querySelector(`#tablaProductosBody tr:contains('${nombreProducto}')`);
        filaEliminar.remove();

        // Actualiza la información del carrito y el monto total
        updateMontoTotal();
        updateProductosArray(nombreProducto, 0, /* Precio del producto */ );

        // Actualiza el campo oculto con la información en formato JSON
        updateProductosInput();
    }

    function actualizarCantidad(nombreProducto, nuevaCantidad, precioUnitario) {
        // Actualiza la cantidad en el array de productos
        updateProductosArray(nombreProducto, nuevaCantidad, precioUnitario);
        ...
        // Actualiza el precio total de unidad
        const precioTotalElement = document.getElementById(`precioTotal_${nombreProducto}`);
        const nuevoPrecioTotal = (nuevaCantidad * precioUnitario).toFixed(2);
        precioTotalElement.textContent = `$${nuevoPrecioTotal}`;
    }

    function pagar() {
        // Habilita el formulario de pago
        const pagoFormContainer = document.getElementById('pagoFormContainer');
        pagoFormContainer.style.display = 'block';

        // Deshabilita el botón de agregar al carrito
        const botonAgregarCarrito = document.getElementById('botonCrearOrden');
        botonAgregarCarrito.disabled = true;

        // Deshabilita los botones de eliminar y actualizar cantidad
        const botonesEliminar = document.querySelectorAll('#tablaProductosBody td button');
        botonesEliminar.forEach(boton => boton.disabled = true);

        const inputsCantidad = document.querySelectorAll('#tablaProductosBody td input');
        inputsCantidad.forEach(input => input.disabled = true);
    }
</script>
