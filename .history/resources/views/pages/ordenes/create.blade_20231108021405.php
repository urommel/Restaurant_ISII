<x-app-layout>
    ...
    <li class="mr-2">
        <button id="{{ $categoria->id }}-tab" data-tabs-target="#{{ $categoria->id }}" type="button" role="tab"
            aria-controls="{{ $categoria->id }}" aria-selected="false"
            class="inline-block p-4 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-gray-300">{{ $categoria->nombre }}</button>
    </li>
    @endforeach
</ul>
<div id="defaultTabContent">
    @foreach ($categorias as $categoria)
    <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="{{ $categoria->id }}"
        role="tabpanel" aria-labelledby="{{ $categoria->id }}-tab">
        <div class="slick-carousel">
            @foreach ($categoria->productos as $producto)
            <!-- Aquí va tu código para mostrar cada producto -->
            @endforeach
        </div>
    </div>
    @endforeach
