            ...
            <div class="mb-8 md:col-span-2" style="height: calc(100vh - 150px); overflow-y: scroll;">
                ...
                <div class="divide-y-2">
                                ...
                                <div class="slick-carousel">
                                    @foreach ($productos as $producto)
                                        @if ($producto->categoria->nombre === 'Entrantes')
                                            <div class="mb-8 producto-container bg-red-50" data-producto-id="{{ $producto->id }}">
                    ...
                    <form action="{{ route('orden.store') }}" method="POST" role="form">
                        ...
                        <input type="hidden" name="numero_mesa" value="{{ $mesas->numero_mesa }}">
                ...
                <div class="p-4 bg-white rounded-lg dark:bg-gray-800">
                    <div class="px-4 py-3 bg-transparent border-b card-header">
                        <h5 class="mb-0 text-16" name="numero_mesa">Mesa: <span
                                class="float-right">0{{ $mesas->numero_mesa }}</span>
                        </h5>
                    </div>
                    <div class="px-4 py-3 bg-transparent border-b card-header">
                        <label for="client_id">Selecciona el cliente:</label>
                        <select name="client_id" id="client_id" {{ old('general_customer') ? 'disabled' : '' }}>
...
{{-- <script>
    ...
    function addToCart(nombre, precio, id) {
        ...
        enlaceEliminar.addEventListener('click', function() {
            // Eliminar la fila al hacer clic en el enlace "Eliminar"
            fila.remove();
...
<script>
    ...
    function addToCart(nombre, precio) {
        ...
        productos.push({
            ...
            precioTotal: parseFloat(montoTotal)
        });
        ...
        // Imprimir información del producto en la consola para verificar
        console.log("Producto añadido:", productos);
        
        // Realizar la operación en la base de datos (aquí deberías hacer la petición al controlador)
        guardarProductoEnBaseDeDatos(nombre, cantidad, precio, parseFloat(precioTotal), parseFloat(montoTotal));
    }
    ...
    function guardarProductoEnBaseDeDatos(nombre, cantidad, precioUnitario, precioTotalPorUnidad, precioTotal) {
        // En esta función, deberías hacer la petición AJAX al controlador para almacenar la información en la base de datos
        // Puedes utilizar fetch o axios para realizar la petición
        // Aquí un ejemplo con fetch (puedes ajustarlo según tu backend)
        fetch('/guardar_producto', {
            ...
            body: JSON.stringify({
                ...
                precioTotalPorUnidad: precioTotalPorUnidad,
