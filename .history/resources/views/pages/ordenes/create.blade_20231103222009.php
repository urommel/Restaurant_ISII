                            ...
                            <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="about"
                                ...
                                <div class="flex flex-col items-center sm:flex-row">
                                    <a href="#" class="flex-shrink-0">
                                        <img class="w-24 h-24 p-2 rounded-lg sm:w-32 sm:h-32" src="/docs/images/products/apple-watch.png" alt="product image" />
                                    </a>
                                    <a href="#" class="w-40 mt-4 overflow-hidden sm:mt-0 sm:ml-6">
                                        <h5 class="text-lg font-semibold text-center text-gray-900 dark:text-white overflow-ellipsis">
                                            {{ $plato->nombre }}
                                        </h5>
                                    </a>
                                    <span class="mt-4 text-xl font-bold text-gray-900 sm:mt-0 sm:ml-6 dark:text-white">${{ $plato->precio }}</span>
                                    <input type="number" id="quantity_{{ $plato->id }}" min="1" value="1" class="w-16 px-2 py-1 mt-4 text-sm text-center text-gray-900 bg-white border border-gray-300 rounded-lg sm:mt-0 sm:ml-6 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400">
                                    <button onclick="agregarAlCarrito('{{ $plato->nombre }}', {{ $plato->precio }}, document.getElementById('quantity_{{ $plato->id }}').value)" class="mt-4 sm:mt-0 sm:ml-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add</button>
                                </div>
                            ...
                            <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="services"
                                ...
                                @foreach ($bebidas as $bebida)
                                    <div class="flex items-center">
                                        <a href="#" class="flex-shrink-0">
                                            <img class="w-24 h-24 p-2 rounded-lg"
                                                src="/docs/images/products/apple-watch.png" alt="product image" />
                                        </a>
                                        <a href="#" class="w-40 ml-6 overflow-hidden">
                                            <h5
                                                class="text-lg font-semibold text-center text-gray-900 dark:text-white overflow-ellipsis">
                                                {{ $bebida->nombre }}
                                            </h5>
                                        </a>
                                        <span
                                            class="ml-6 text-xl font-bold text-gray-900 dark:text-white">${{ $bebida->precio }}</span>
                                        <input type="number" id="quantity_{{ $bebida->id }}" min="1"
                                            value="1"
                                            class="w-16 px-2 py-1 ml-6 text-sm text-center text-gray-900 bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400">
                                        <button
                                            onclick="agregarAlCarrito('{{ $bebida->nombre }}', {{ $bebida->precio }}, document.getElementById('quantity_{{ $bebida->id }}').value)"
                                            class="ml-auto text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add</button>
                                    </div>
                                @endforeach
