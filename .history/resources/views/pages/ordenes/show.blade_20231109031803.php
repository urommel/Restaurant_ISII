...
Mis disculpas por la omisión. Para actualizar el "Precio total de unidad" cuando se cambia la cantidad, podemos ajustar el script de la siguiente manera:
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody id="tablaProductosBody">
            ...
            @foreach ($orden->detalles as $detalle)
                <tr>
                    <td>
                        <button onclick="eliminarProducto('{{ $detalle->producto_nombre }}')">Eliminar</button>
                    </td>
                    <td>{{ $detalle->producto_nombre }}</td>
                    <td>
                        <input type="number" id="quantity_{{ $detalle->producto_nombre }}" name="cantidades[]"
                               min="1" value="{{ $detalle->cantidad }}" class="w-16 px-2 py-1 ml-4 text-sm text-gray-900
                               bg-white border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-700
                               dark:text-gray-400" oninput="actualizarCantidad('{{ $detalle->producto_nombre }}', this.value, {{ $detalle->precio_unitario }})">
                    </td>
                    <td>${{ $detalle->precio_unitario }}</td>
                    <td id="precioTotal_{{ $detalle->producto_nombre }}">${{ number_format($detalle->precio_total, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
<script>
    ...
    function actualizarCantidad(nombreProducto, nuevaCantidad) {
        ...
        updateProductosArray(nombreProducto, nuevaCantidad, /* Precio del producto */ );
