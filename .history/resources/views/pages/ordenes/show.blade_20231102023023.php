<x-app-layout>
    <div class="w-full px-4 py-8 mx-auto sm:px-6 lg:px-8 max-w-9xl">
        ...
        <p>Mesa: {{ $orden->mesa->numero_mesa }}</p>

        <h2>Bebidas</h2>
        @foreach ($orden->bebidas as $bebida)
            <p>{{ $bebida->nombre }} - Cantidad: {{ $bebida->pivot->cantidad }} - Precio: {{ $bebida->pivot->precio }}
            </p>
        @endforeach
        ...
        <button onclick="confirmarPedido({{ $orden->id }})" class="px-4 py-2 text-white bg-green-500">Confirmar
            Pedido</button>
