            ...
            <div class="mb-8 md:col-span-2" style="height: calc(100vh - 150px); overflow-y: scroll;">
                ...
                <div class="">
                    <div class="divide-y-2">
                        ...
                        <button type="button" id="botonCrearOrden" class="w-full mt-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            Crear Orden
                        </button>
                    </div>
                </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        // Esperar a que se cargue el DOM antes de agregar eventos
        const botonCrearOrden = document.getElementById("botonCrearOrden");
        if (botonCrearOrden) {
            botonCrearOrden.addEventListener("click", crearOrden);
        }
    });
        ...
        fetch("{{ route('ordenes.store') }}", {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const numeroOrdenElement = document.getElementById('numeroOrden');
                    numeroOrdenElement.textContent = data.orden.id;

                    // Mostrar el detalle de la orden
                    const detalleOrdenContainer = document.getElementById('detalleOrdenContainer');
                    detalleOrdenContainer.style.display = 'block';

                    // Agregar filas al detalle de la orden
                    const detalleOrdenBody = document.getElementById('detalleOrdenBody');
                    ...
                    productos.forEach(producto => {
                        ...
                        fila.appendChild(celdaProducto);
                        fila.appendChild(celdaCantidad);
                        detalleOrdenBody.appendChild(fila);
                    });
                } else {
