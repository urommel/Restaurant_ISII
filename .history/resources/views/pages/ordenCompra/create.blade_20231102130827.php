        ...
        <form action="{{ route('ordenes-compra.store') }}" method="POST">
            ...
            <div class="grid grid-cols-3 gap-4">
                @foreach($productos as $producto)
                    <div>
                        <label for="detalles[{{ $producto->id }}][cantidad]" class="block text-sm font-medium text-gray-600">{{ $producto->nombre }}</label>
                        <input type="number" name="detalles[{{ $producto->id }}][cantidad]" class="mt-1 p-2 border rounded w-full">
                    </div>
                @endforeach
            </div>
    
            <button type="submit" class="mt-4 bg-blue-500 text-white py-2 px-4 rounded">Guardar Orden de Compra</button>
        </form>
