                ...
                <ul>
                    @foreach ($productos as $producto)
                        <li class="mb-4">
                            ...
                            <p><strong>Tipo de Plato:</strong> {{ $producto->tipoPlato }}</p>
                            <p><strong>Precio:</strong> ${{ $producto->precio }}</p>
                            {{-- <img src="{{ asset('storage/' . $plato->urlPlatos) }}" alt="{{ $plato->nombre }}" class="max-w-full mt-4"> --}}
                            <img src="{{ $plato->urlPlatos }}" alt="{{ $producto->nombre }}" class="max-w-full mt-4">

                            <div class="mt-4">
                                <a href="{{ route('productos.edit', $plato->id) }}" class="text-blue-500">Editar</a>
                                <form action="{{ route('productos.destroy', $plato->id) }}" method="post" class="inline">
                                    @csrf
                                    @method('DELETE')
